...
`.trim();
  ...
  const exportToDevice = async () => {
    ...
    try {
      ...
      Alert.alert('Export Failed', 'Failed to export material inspections');
    } finally {
      setLoading(false);
      setProgress(0);
    }
  ...
  const exportViaEmail = async () => {
    try {
      ...
      // Individual reports (limit to 5 to avoid email size limits)
      for (const cargo of cargosToExport.slice(0, 5)) {
        const reportContent = generateTxtContent(cargo);
  ...
  const exportViaWhatsApp = async () => {
    try {
      ...
      // Create a simple text summary for WhatsApp
      ...
      Alert.alert('WhatsApp Failed', 'Failed to open WhatsApp');
    }
  };
